...
import { Camera, Upload, Plus, X, Phone } from "lucide-react";
...
export const Submit = () => {
  const [formData, setFormData] = useState({
    ...
    tiktok_title: "",
    tiktok_url: "",
    // If you have a user ID (optional)
    // user_id: "",
  });
  ...
  useEffect(() => {
    ...
    // Move marker when user clicks on map
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submissionPayload = {
        ...
        coords: [parseFloat(formData.latitude), parseFloat(formData.longitude)],
        images: imagePaths,
      ...
      // increment pending submissions in Zustand store
      useUserStore.getState().incrementPending();
      ...
      if (marker.current && map.current) {
        marker.current.setLngLat([36.01, 34.25]);
        map.current.flyTo({ center: [36.01, 34.25], zoom: 8 });
      }
    } catch (error) {
